...
export function HelperGame({ inviteId }: HelperGameProps) {
  ...
  if (!hasJoined) {
    return (
      <div className="text-center space-y-6">
        <div className="text-2xl font-bold text-black">You're Invited to Help!</div>
        <p className="text-gray-600">
          <span className="font-semibold">{inviteInfo.creatorName}</span> needs your help guessing their impossible word!
        </p>
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <div className="text-2xl font-bold text-black mb-2">Helping {inviteInfo.creatorName}</div>
        <p className="text-gray-600">You can suggest up to 3 words to help them guess!</p>
      </div>

      <div className="bg-gray-100 rounded-lg p-4">
        <p className="text-sm text-gray-600 mb-2 text-center">Letters in the word:</p>
        <div className="flex justify-center gap-1 flex-wrap">
          {helperGameWord.shuffledLetters?.map((letter, index) => (
            <span
              key={index}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <input
              type="text"
              value={currentInput}
              onChange={(e) => setCurrentInput(e.target.value.toLowerCase().replace(/[^a-z]/g, '').slice(0, helperGameWord.word.length))}
              ...
              maxLength={helperGameWord.word.length}
            />
      ...
      {helperState && !helperState.canSuggest && !helperState.mainUserGameOver && (
        <div className="text-center space-y-4">
          <div className="text-lg font-semibold text-gray-600">All suggestions used!</div>
          <p className="text-gray-500">You've sent all 3 suggestions. Good luck to your friend!</p>
        </div>
      )}
    </div>
  );
