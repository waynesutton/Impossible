...
- **ImpossibleCrossword.tsx**: Main crossword puzzle component featuring 7x7 grid layout, simplified static hints display, manual word submission system, friend collaboration with unlimited suggestions, grid state management, word selection highlighting, and unlimited play with "Start Over" functionality.
...
- **crossword.ts**: Crossword mode logic for weekly AI-generated puzzle management. Includes:

  - Weekly word pool generation (80 themed words) using GPT-4o-mini AI via cron job
  - Algorithmic layout system with deterministic word placement and guaranteed intersections
  - Smart word selection prioritizing compatible words with shared letters
  - 7x7 grid with 3-4 words ensuring reliable gameplay and proper crossword structure
  - Real-time progress tracking and grid state synchronization
  - Friend collaboration system with unlimited suggestions and instant application
  - Crossword invitation management and helper integration with live updates
  - Manual word submission system with word selection and validation
  - Grid layout validation and word position management with fallback system
...
- **schema.ts**: Complete database schema definitions using Convex validators. Defines tables for:
  ...
  **Crossword Mode:**
  ...
  - `wordPool`: Weekly themed word pool (80 words) generated by AI cron job every Sunday
  - `crosswordPuzzles`: AI-generated 7x7 crossword puzzles with algorithmic layout and guaranteed intersections
  - `userCrosswordAttempts`: Progress tracking with manual submission system and word selection state
  - `crosswordInvites`: Shareable invitation links for friend collaboration with real-time sync
  - `crosswordSuggestions`: Unlimited friend suggestions with instant grid application
...
- `convex/crossword.ts`: Weekly word pool generation using GPT-4o-mini AI, algorithmic layout system with deterministic word placement, smart word selection for guaranteed intersections, friend collaboration with unlimited suggestions
- `src/ImpossibleCrossword.tsx`: Simplified static hints display, manual word submission system, word selection highlighting, friend suggestion integration with instant grid application
